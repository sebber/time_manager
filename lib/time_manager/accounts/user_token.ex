defmodule TimeManager.Accounts.UserToken do
  ...
  def build_session_token(user) do
    ...
    {token, %TimeManager.Accounts.UserToken{token: token, context: "session", user_id: user.id}}
  end
  ...
  defp build_hashed_token(user, context, sent_to) do
    ...
    {Base.url_encode64(token, padding: false),
     %TimeManager.Accounts.UserToken{
       token: hashed_token,
       context: context,
       sent_to: sent_to,
       user_id: user.id
  ...
  def user_and_contexts_query(user, :all) do
    from t in TimeManager.Accounts.UserToken, where: t.user_id == ^user.id
  end

  def user_and_contexts_query(user, [_ | _] = contexts) do
    from t in TimeManager.Accounts.UserToken, where: t.user_id == ^user.id and t.context in ^contexts
  end
end
